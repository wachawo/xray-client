...
def detect_interface() -> str:
    ...
    try:
        line = run(
            ["ip", "route", "get", "8.8.8.8"], dry_run=False, check=True
        ).splitlines()[0]
    ...
    line = run(
        ["ip", "route", "show", "default", "0.0.0.0/0"], dry_run=False, check=True
    )
    m = re.search(r" dev (\S+)", line)
    if not m:
...
def detect_addr_prefix(iface: str) -> Tuple[str, int]:
    out = run(
        ["ip", "-o", "-4", "addr", "show", "dev", iface, "scope", "global", "primary"],
        dry_run=False,
        check=True,
    )
...
def render_template(
    env_vars: Dict[str, str], servers: List[Dict[str, str]], dry_run: bool
):
    ...
    domain_outbound_tag = (
        tagged_servers[1]["tag"]
        if len(tagged_servers) > 1
        else tagged_servers[0]["tag"]
    )
...
def install_docker(dry_run: bool):
    ...
    if distro == "ubuntu":
        ...
        codename = os_release.get("UBUNTU_CODENAME") or os_release.get(
            "VERSION_CODENAME"
        )
        if not codename:
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force re-detect and overwrite .env"
    )
    parser.add_argument(
        "--server",
        action="append",
